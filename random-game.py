...
parser.add_argument("-steamuser", metavar="steamuser", default="fackingtroll", type=str, help="Enter your Steam username")
parser.add_argument("-amount", metavar="amount", default=10, type=int, help="Enter amount of games to choose")
parser.add_argument("-open", metavar="open", default="y", type=str, help="Enter 'y' if you want to open links")
args = parser.parse_args()

def get_steam_id():
    steam_api_get_id = f"https://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key={api_keys.STEAM_API}&vanityurl={steam_username}"
...
def get_steam_game_list():
    steam_api_call = f"https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key={api_keys.STEAM_API}&steamid={steam_id}&include_appinfo=true&format=json"
    request = requests.get(steam_api_call)
    full_json = request.json()
    all_games = full_json["response"]["games"]
    for game in all_games:
        game_id = game["appid"]
        all_steam_games[game["name"]] = f"https://store.steampowered.com/app/{game_id}/"
...
def main():
    ...
    for _ in range(games_amount):
        ...
        if platform == "steam":
            ...
            rawg_api_call = f"https://api.rawg.io/api/games?key={api_keys.RAWG_API}&page={page}&page_size=1"
